...
(() => {
  ...
  function M(t, e) {
    ...
    if (
      ...
      for (let [r, l] of Object.entries(e.hiddenFields)) o.set(r, l);
    return (
      new URLSearchParams(window.location.search).forEach((r, l) => {
        o.has(l) || o.set(l, r);
      }),
  ...
  function v(t, e, n) {
    ...
    ((a.className = `bf-popup bf-popup--${o ? "center" : r}`),
      (a.style.width = `${l}px`),
    ...
    return (
      e.emoji?.text &&
        ...
        (c.className = `bf-emoji ${A(e.emoji.animation)}`),
      ...
      { overlay: s, popup: a, iframeContainer: f, closeBtn: p, loadingEl: d, emojiEl: c }
    );
  }
  function w(t) {
...
`;
  ...
  function S(t) {
    ...
    r && (e.emoji = { text: r, animation: l || "none" });
  ...
  function k(t) {
    document.addEventListener("click", (e) => {
      ...
      if (i?.href?.includes("form-open=")) {
        ...
        if (r !== -1) {
          let l = i.href.substring(r),
            { formId: s, options: a } = g(l);
          s && t(s, a);
        }
      }
    });
  ...
  function u(t, e = {}) {
    ...
    if (
      ...
      try {
        ...
        console.error("[BetterForms] onOpen callback error:", r);
      }
  }
  function b(t) {
    let e = m.get(t);
