...
function TimePickerInputGroup(props: DivProps) {
  ...
  const inputRefsMap = useLazyRef(() => new Map<Segment, React.RefObject<InputElement | null>>());
...
type TimePickerContentProps = React.ComponentProps<typeof PopoverContent>;
...
function TimePickerContent(props: TimePickerContentProps) {
  ...
  const columnsRef = useLazyRef(() => new Map<string, Omit<ColumnData, "id">>());
  ...
  React.useEffect(() => {
    ...
    // Defer to next frame so columns have registered
  ...
  // Handle clicks outside: prevent close when clicking inside input group
  React.useEffect(() => {
    if (!openOnFocus || !inputGroupRef.current) return;

    const handlePointerDown = (event: PointerEvent) => {
      const target = event.target;
      if (!(target instanceof Node)) return;
      if (inputGroupRef.current?.contains(target)) {
  ...
  const onKeyDown = React.useCallback(
    (event: React.KeyboardEvent<ColumnItemElement>) => {
      ...
      if (event.key === "ArrowUp" || event.key === "ArrowDown") {
        event.preventDefault();
        const items = columnContext.getItems().toSorted((a, b) => {
