  ...
  return (
    <div className="flex flex-col border-b border-foreground/5 last:border-0">
      <button
        ...
        className="flex items-center justify-between py-3 group hover:text-foreground transition-colors"
...
function SocialButton({ label, icon }: { label: string; icon: React.ReactNode }) {
  return (
    <button
      type="button"
      className="flex items-center justify-center gap-2 w-full bg-secondary text-secondary-foreground py-2.5 rounded-full text-[14px] font-bold hover:bg-secondary/40 transition-colors border border-foreground/5"
    >
      {icon}
      {label}
    </button>
