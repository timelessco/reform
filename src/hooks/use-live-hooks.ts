import { eq, not } from "@tanstack/react-db";
import {
    editorDocCollection,
    formCollection,
    localFormCollection,
...
export const useFormsForWorkspace = (workspaceId?: string) => {
    const { data: formsData } = useLiveQuery((q) => {
        ...
        query = query.where(({ form }) => eq(form.status, "draft") || eq(form.status, "published"));
...
export const useForms = () => {
    const { data: formsData } = useLiveQuery((q) =>
        q
            ...
            .where(({ form }) => eq(form.status, "draft") || eq(form.status, "published"))
            .select(({ form }) => ({
                id: form.id,
                title: form.title,
                workspaceId: form.workspaceId,
