...
function WorkspaceDashboard() {
  ...
  const paginatedForms = sortedForms.slice(startIndex, startIndex + FORMS_PER_PAGE);
  ...
  const handleCreateWorkspace = async () => {
    ...
    try {
      const newWs = await createWorkspaceLocal(workspace.organizationId, "Collection");
      navigate({
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between pt-4 border-t">
              <p className="text-sm text-muted-foreground">
                Showing {startIndex + 1}-{Math.min(startIndex + FORMS_PER_PAGE, sortedForms.length)}{" "}
                of {sortedForms.length} forms
              </p>
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-4">
                <p>
                  This will permanently delete <strong>"{workspace?.name}"</strong> and all forms
