...
import { ChevronDown, Clock, Globe, MousePointer2, Search, User, Users } from "lucide-react";
...
function InsightsPage() {
  const [activeSubTab, setActiveSubTab] = useState<"visits" | "drop-off">("visits");
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-auto">
      <div className="max-w-[1200px] mx-auto w-full p-8 md:p-12 space-y-8">
        {/* Secondary Header */}
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" className="h-8 gap-2 bg-background border-muted/60">
        ...
        <div className="border border-dashed rounded-2xl min-h-[300px] flex items-center justify-center bg-muted/5">
          <div className="text-center space-y-1">
            <p className="text-sm font-medium text-muted-foreground/60">No data</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
          {analyticsGrids.map((grid) => (
            <div key={grid.title} className="space-y-4">
              <h3 className="text-sm font-semibold text-muted-foreground/80 lowercase tracking-tight">
                <span className="capitalize">{grid.title}</span>
              </h3>
        ...
        <div className="pt-12 pb-8 border-t flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-[11px] text-muted-foreground leading-relaxed text-center md:text-left max-w-2xl">
            Privacy-friendly and fully anonymous analytics — no cookies, no personal data, and no
