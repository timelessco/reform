...
function MyAccountPage() {
    ...
    const updateProfileMutation = useMutation(auth.updateUser.mutationOptions({
        onSuccess: () => {
    ...
    const enable2faMutation = useMutation(auth.twoFactor.enable.mutationOptions({
        onSuccess: () => {
    ...
    const verifyTotpMutation = useMutation(auth.twoFactor.verifyTotp.mutationOptions({
        onSuccess: () => {
    ...
    const disable2faMutation = useMutation(auth.twoFactor.disable.mutationOptions({
        onSuccess: () => {
    ...
    const unlinkAccountMutation = useMutation(auth.unlinkAccount.mutationOptions({
        onSuccess: (_, variables) => {
            toast.success(`${variables.providerId} disconnected`)
            queryClient.invalidateQueries({ queryKey: auth.listAccounts.queryKey() })
        },
        onError: (error: any) => {
    ...
    const deleteAccountMutation = useMutation(auth.deleteUser.mutationOptions({
        onSuccess: () => {
    ...
    return (
        <div className="max-w-xl space-y-12 pb-24">
            ...
            <section className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative">
                            <Input value={user?.email || ""} readOnly className="pr-24 bg-muted/30" />
                            <Button variant="link" className="absolute right-0 top-0 h-full px-4 text-xs">Change email</Button>
                        </div>
                    </div>
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-muted-foreground">Password</label>
                        <div className="relative">
                            <Input placeholder="Not set" readOnly className="pr-24 bg-muted/30" />
                            <Button variant="link" className="absolute right-0 top-0 h-full px-4 text-xs">Set password</Button>
                        </div>
                    </div>
                </div>
                ...
                >
                    {updateProfileMutation.isPending ? <Loader2 className="animate-spin mr-2 h-4 w-4" /> : null}
                    Update
                </Button>
            </section>
            ...
            <section className="space-y-8 pt-8 border-t">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            {user?.twoFactorEnabled ? (
                                <Badge className="bg-green-500 hover:bg-green-600 text-[10px] uppercase text-white border-0">Enabled</Badge>
                            ) : (
                                <Badge variant="secondary" className="font-normal text-[10px] uppercase">Disabled</Badge>
                            )}
                        </div>
                    </div>
                    <p className="text-sm text-muted-foreground max-w-md">
                        Secure your account with two-factor authentication which adds an additional layer of security during login.
                    </p>

                    {user?.twoFactorEnabled ? (
                        <Button variant="outline" size="sm" onClick={handleDisable2fa} className="text-destructive hover:text-destructive font-medium border-destructive/20">
                            Disable 2FA
                        </Button>
                    ) : (
                        <Dialog open={is2faDialogOpen} onOpenChange={(open) => {
                            ...
                            <DialogTrigger asChild>
                                <Button variant="outline" size="sm" onClick={handleStart2faSetup} className="bg-black text-white hover:bg-black/80 font-medium">
                                    Set up
                                </Button>
                            </DialogTrigger>
                                ...
                                <div className="py-4 space-y-6">
                                    {twoFaStep === 1 && (
                                        <div className="space-y-4">
                                            ...
                                            >
                                                {setup2faMutation.isPending ? <Loader2 className="animate-spin mr-2 h-4 w-4" /> : null}
                                                Next
                                            </Button>
                                        </div>
                                    )}
                                    ...
                                    {twoFaStep === 2 && (
                                        <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                                            <div className="space-y-2">
                                                <p className="text-sm font-medium">2. Scan the QR code with your authenticator app</p>
                                            ...
                                            <Button
                                                ...
                                                disabled={otpCode.length < 6 || verifyTotpMutation.isPending || enable2faMutation.isPending}
                                            >
                                                {verifyTotpMutation.isPending || enable2faMutation.isPending ? <Loader2 className="animate-spin mr-2 h-4 w-4" /> : null}
                                                Enable 2FA
                                            </Button>
                                        </div>
                                    )}
                ...
                <div className="space-y-4 pt-4 border-t">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Connect your account with Google or Apple to enable faster, secure and more convenient access.
                    </p>
                    ...
                    <div className="space-y-4 pt-2">
                        ...
                        <div className="flex items-center justify-between group opacity-50 contrast-50 pointer-events-none">
                            ...
                            <Button variant="ghost" size="sm" className="text-muted-foreground font-normal">Coming soon</Button>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section className="space-y-4 pt-12 border-t">
                ...
                >
                    {deleteAccountMutation.isPending ? <Loader2 className="animate-spin mr-2 h-4 w-4" /> : null}
                    Delete account
                </Button>
            </section>
        </div>
